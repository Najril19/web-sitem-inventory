...
function initializeDatabase(dbPath: string): Database.Database {
  ...
  const checkUser = database.prepare('SELECT id FROM users WHERE username = ?').get('admin');
  ...
  const supplierCount = database.prepare('SELECT COUNT(*) as count FROM supplier').get() as { count: number };
  if (supplierCount.count === 0) {
    ...
    const insertSupplier = database.prepare('INSERT INTO supplier (nama, kontak, alamat, keterangan) VALUES (?, ?, ?, ?)');
    suppliers.forEach(s => insertSupplier.run(...s));
    console.log(`✓ Inserted ${suppliers.length} sample suppliers`);
  }

  // Insert sample barang
  const barangCount = database.prepare('SELECT COUNT(*) as count FROM barang').get() as { count: number };
  if (barangCount.count === 0) {
    ...
    const insertBarang = database.prepare(`
  ...
  const already = database.prepare('SELECT 1 FROM schema_migrations WHERE name = ?').get(migrationName);
  if (!already) {
    database.prepare('DELETE FROM transaksi_masuk').run();
    database.prepare('DELETE FROM transaksi_keluar').run();
    database.prepare('UPDATE barang SET stok = 0, updated_at = CURRENT_TIMESTAMP').run();
