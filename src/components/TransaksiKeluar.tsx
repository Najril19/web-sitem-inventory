...
import { Plus, TrendingDown, Calendar, Package, User, ArrowUpCircle, CheckCircle, Clock, ShoppingCart, Search, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
  ...
  const fetchTransaksi = async () => {
    try {
      ...
      if (result.success) {
        ...
        showError('Gagal Memuat Data', result.message || 'Tidak dapat memuat data transaksi keluar');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedBarang && formData.jumlah > selectedBarang.stok) {
      showError(
        ...
        `Stok ${selectedBarang.nama_barang} hanya tersisa ${selectedBarang.stok} unit, tidak dapat mengeluarkan ${formData.jumlah} unit`
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        const tujuanText = formData.tujuan ? ` untuk ${formData.tujuan}` : '';
        showSuccess(
          'Transaksi Berhasil!', 
          `${formData.jumlah} unit ${selectedBarang?.nama_barang || 'barang'}${tujuanText} berhasil dikeluarkan`
        );
      } else {
      ...
      console.error('Error saving transaksi keluar:', error);
      showError('Error Koneksi', 'Tidak dapat terhubung ke server. Periksa koneksi internet Anda.');
    } finally {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Cari nama barang, tujuan, atau keterangan..."
            value={searchTerm}
            onChange={(e) => {
              setSearchTerm(e.target.value);
              setCurrentPage(1);
            }}
            className="w-full pl-10 pr-4 py-3 border border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-gray-700 text-white"
          />
        </div>
      </motion.div>
                    ...
                    <td className="px-6 py-4 text-gray-300">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 bg-orange-500/20 rounded-full flex items-center justify-center border border-orange-500/30">
                          <span className="text-xs text-orange-400 font-medium">{startIndex + index + 1}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-300">
                  ...
                  <motion.button
                    ...
                    className={`flex-1 py-3 rounded-lg font-medium shadow-lg transition-all flex items-center justify-center gap-2 ${
                      isSubmitting 
                        ...
                        : 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-orange-500/30 hover:shadow-xl'
                    }`}
                  >
                    {isSubmitting ? (
                      <>
